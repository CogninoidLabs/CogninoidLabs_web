...
export default function NextStepBox({ title = "Next Steps", steps = [] }: NextStepBoxProps) {
    ...
    <div className="my-8 bg-[#07070f] border border-cyan-500/20 rounded-2xl p-6 relative overflow-hidden">
      ...
      <div className="flex flex-col gap-2.5">
        {steps.map((step) => (
          ...
          >
            <div>
              <p className="text-white text-sm font-medium group-hover:text-cyan-400 transition-colors">{step.label}</p>
              {step.description && (
                <p className="text-slate-600 text-xs mt-0.5">{step.description}</p>
              )}
            </div>
